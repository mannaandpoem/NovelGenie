...
1. 先识别出原文不足或需要优化的行或段落，然后进行润色和扩展。
2. 请根据行号准确定位需要修改的行或段落，避免在段落之间进行混乱的修改。
3. 行号识别：你需要在操作时，准确识别并标明行或段落的起止行号。每个修改都应根据每一行的内容合理分配，避免跨段修改，确保修改后的内容与上下文协调一致。
4. 对每个需要优化的行或段落，使用以下格式进行编辑：
...
# 使用被 ```python ...``` 三引号包裹的字符串的 cmds 列表组织所有的编辑命令
```python
cmds = [
    # 第一部分：增强开场与修炼描写
    '''edit 1:1 <<EOF
